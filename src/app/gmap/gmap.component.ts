...
export class GMapComponent implements OnInit {
  ...
  public clct(a) {
    this.afs
      .collection(`locations`, (ref) => ref.where('Nama', '==', a))
      .valueChanges()
      .subscribe((res) => {
        this.loadedData = res;
        // var ancok=[];
  ...
  public addMarker() {
    const map = this.gmApi.createMap(
    ...
    for (let i = 0; i < 3; i++) {
      console.log(
        this.loadedData[i].coords.latitude,
        this.loadedData[i].coords.longitude
      );
      const mrkr = this.gmApi.createMarker({
        position: {
          lat: this.loadedData[i].coords.latitude,
          lng: this.loadedData[i].coords.longitude,
        },
        clickable: true,
      });
